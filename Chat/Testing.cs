...
namespace Chat
{
    internal abstract class Check
    {
        ...
        internal static void CycleCode(byte a, byte n, out byte code)
        {
        ...
        internal static int CountBit(byte a)
        {
            int count = 0;
            while(!(a==0))
            {                
        ...
        internal static bool CycleDecode(byte a,out byte code,byte n)
        {
            while (CountBit(a) != CountBit(n)) n <<= 1;//уравняли по двоичному порядку делимое и делитель
            ///цикла while-необходим для получения остатка от деления 
            ///v(x)*x^n-k на образующий полином.
        ...
        internal static List<byte> CodingStr(string str)
        {
            char[] ChrArr;
        ...
        /// <returns></returns>
        internal static string DeCodingStr(byte[] str)
        {
            char ch;
